...
def decrypt_key(session_key):
    f = open('Keys/Server', 'rb')
    PrivKey = RSA.importKey(f.read())
    f.close()
    return PrivKey.decrypt(session_key)
...
def verify_hash(user, password):
    usr_str = user.decode('utf-8')
    pass_str = password.decode('utf-8')
    try:
        reader = open("passfile.txt", 'r')
        for line in reader.read().split('\n'):
            line = line.split("\t")
            if line[0] == usr_str:
                hashed_pass = hashlib.sha512((pass_str + line[1]).encode()).hexdigest()
                if(hashed_pass == line[2]):
                    return True
                else:
                    return False
        reader.close()
